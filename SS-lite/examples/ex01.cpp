 ...
 *
 * The full library includes built in support for generating models, building databases, classifying solvers
 * and allows for building of models at run time. 
 *
 * Using the library in a Petsc executable is easy. 
 *
 ...
 *
 *   NOTES:
 *       a) the filestem and featureset commands are required -- The program will abort if you choose the KSPSS solver but do not provide them
 ...
 *           the <filestem>.names file must match those in the feature set (with no spelling differences). 
 *
 *           To set up a new feature set, see the README in the Feature sets directory. 
 *   
 *
...
int main(int argc,char **args)
{
  ...
  PetscCoupler::CreateSolverSelectorKSP();
  /************ That is it, everything below here is plain old petsc . ***********/ 
