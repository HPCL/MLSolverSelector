...
const char* sMultOps[] = {"*", "/", "%", (String) 0};
const char* sFuncs[] = {"sin", "cos", "tan", "log", "exp", "int", (const char*) 0};
...
DataRec C50Classifier::GetDataRecFromVec(std::vector<std::string> &datavec, Boolean Train)
/*      ----------  */
{
    Attribute	Att;
    ...
    if ( datavec.size()>0 )
    {
        ...
        ForEach(Att, 1, MaxAtt)
        {
            ...
            if ( ! FirstValue && datavec.size() <= datavecNextIndex )
            {
                XError(HITEOF, AttName[Att], "");
                FreeLastCase(DVec);
                return Nil;
            } else if ( !FirstValue ) {
                name_str = datavec[datavecNextIndex++].c_str();
            }
            FirstValue = false;
            ...
            if ( Exclude(Att) )
            {
                if ( Att == LabelAtt )
                {
                    ...
                    SVal(DVec,Att) = StoreIVal(name_str.c_str());
                }
            }
            else if ( ! strcmp(name_str.c_str(), "?") )
            {
            ...
            else if ( Att != ClassAtt && ! strcmp(name_str.c_str(), "N/A") )
            {
            ...
            else if ( Discrete(Att) )
            {
                ...
                Dv = Which(name_str.c_str(), AttValName[Att], 1, MaxAttVal[Att]);
                if ( ! Dv )
                {
                    if ( StatBit(Att, DISCRETE) )
                    {
                        if ( Train )
                        {
                            ...
                            {
                                Dv = ++MaxAttVal[Att];
                                AttValName[Att][Dv]   = strdup(name_str.c_str());
                    ...
                    {
                        XError(BADATTVAL, AttName[Att], name_str.c_str());
                        Dv = UNKNOWN;
                    }
                }
                DVal(DVec, Att) = Dv;
            ...
            else
            {
                ...
                if ( TStampVal(Att) )
                {
                    CVal(DVec, Att) = Cv = TStampToMins(name_str.c_str());
                    if ( Cv >= 1E9 )	/* long time in future */
                    {
                        XError(BADTSTMP, AttName[Att], name_str.c_str());
                        DVal(DVec, Att) = UNKNOWN;
                    }
                }
                else if ( DateVal(Att) )
                {
                    CVal(DVec, Att) = Cv = DateToDay(name_str.c_str());
                    if ( Cv < 1 )
                    {
                        XError(BADDATE, AttName[Att], name_str.c_str());
                        DVal(DVec, Att) = UNKNOWN;
                    }
                }
                else if ( TimeVal(Att) )
                {
                    CVal(DVec, Att) = Cv = TimeToSecs(name_str.c_str());
                    if ( Cv < 0 )
                    {
                        XError(BADTIME, AttName[Att], name_str.c_str());
                        DVal(DVec, Att) = UNKNOWN;
                    }
                }
                else
                {
                    CVal(DVec, Att) = strtod(name_str.c_str(), &EndName);
                    if ( EndName == name_str.c_str() || *EndName != '\0' )
                    {
                        XError(BADATTVAL, AttName[Att], name_str.c_str());
                        DVal(DVec, Att) = UNKNOWN;
                    }
        ...
        {
            if ( datavec.size() <= datavecNextIndex )
            {
                XError(HITEOF, Fn, "");
                FreeLastCase(DVec);
                return Nil;
            }
            name_str = datavec[datavecNextIndex++];
            Class(DVec) = Dv = Which(name_str.c_str(), ClassName, 1, MaxClass);
        }
...
void C50Classifier::GetNames(FILE *Nf)
/*   --------  */
{
...
void C50Classifier::ExplicitAtt(FILE *Nf)
/*   -----------  */
{
    char	Buffer[1000]="", *p;
    DiscrValue	v;
...
int C50Classifier::Which(String Val, String *List, int First, int Last)
/*  -----  */
{
    int	n=First;
...
int C50Classifier::InChar(FILE *f)
/*  ------  */
{
    if ( ! *LBp )
    {
...
DataRec C50Classifier::GetDataRec(FILE *Df, Boolean Train)
/*      ----------  */
{
    Attribute	Att;
    char	Name[1000], *EndName;
...
int C50Classifier::StoreIVal(String S)
/*  ---------  */
{
    int		StartIx, Length;
...
void C50Classifier::CheckValue(DataRec DVec, Attribute Att)
/*   ----------  */
{
    ContValue	Cv;
...
void C50Classifier::ImplicitAtt(FILE *Nf)
/*   -----------  */
{
#ifdef CUBIST
    _UNK.cval = UNKNOWN;
...
void C50Classifier::ReadDefinition(FILE *f)
/*   --------------  */
{
...
void C50Classifier::Append(char c)
/*   ------  */
{
...
Boolean C50Classifier::Expression()
/*      ----------  */
{
    int		Fi=BN;
...
Boolean C50Classifier::Conjunct()
/*      --------  */
{
    int		Fi=BN;
...
Boolean C50Classifier::SExpression()
/*      -----------  */
{
    int		o, Fi=BN;
...
Boolean C50Classifier::AExpression()
/*      -----------  */
{
    int		o, Fi=BN;
...
Boolean C50Classifier::Find(String S)
/*      ----  */
{
    if ( Buff[BN] == ' ' ) BN++;
...
int C50Classifier::FindOne(String *Alt)
/*  -------  */
{
    int	a;
...
Attribute C50Classifier::FindAttName()
/*        -----------  */
{
...
void C50Classifier::DefSyntaxError(String Msg)
/*   --------------  */
{
    char*	RestOfText;
    int		i=10;
...
void C50Classifier::DefSemanticsError(int Fi, String Msg, int OpCode)
/*   -----------------  */
{
...
void C50Classifier::Dump(char OpCode, ContValue F, String S, int Fi)
/*   ----  */
{
    if ( Buff[Fi] == ' ' ) Fi++;
...
void C50Classifier::DumpOp(char OpCode, int Fi)
/*   ------  */
{
...
Boolean C50Classifier::UpdateTStack(char OpCode, ContValue F, String S, int Fi)
/*      ------------  */
{
    if ( TSN >= TStackSize )
    {
...
AttValue C50Classifier::EvaluateDef(Definition D, DataRec Case)
/*       -----------  */
{
...
void C50Classifier::ReadFilePrefix(String Extension)
/*   --------------  */
{
...
Tree C50Classifier::GetTree(String Extension)
/*   -------  */
{
...
CRuleSet C50Classifier::GetRules(String Extension)
/*	 --------  */
{
    CheckFile(Extension, false);
...
CRuleSet C50Classifier::InRules()
/*	 -------  */
{
    CRuleSet	RS;
    RuleNo	r;
...
Condition C50Classifier::InCondition()
/*        -----------  */
{
    Condition	C;
    char	Delim;
...
void C50Classifier::ConstructRuleTree(CRuleSet RS)
/*   -----------------  */
{
    int		r, c;
    RuleNo	*All;
...
void C50Classifier::SetTestIndex(Condition C)
/*   ------------  */
{
    int		t;
    Condition	CC;
...
RuleTree C50Classifier::GrowRT(RuleNo *RR, int RRN, CRule *Rule)
/*       ------  */
{
    RuleTree	Node;
    RuleNo	r, *LR;
...
int C50Classifier::DesiredOutcome(CRule R, int TI)
/*  --------------  */
{
    int		c;
    Boolean	ContinTest;
...
int C50Classifier::SelectTest(RuleNo *RR, int RRN, CRule *Rule)
/*  ----------  */
{
    int		c, cc, ri;
    RuleNo	r;
...
int C50Classifier::ReadProp(char *Delim)
/*  --------  */
{
    int		c, i;
    char	*p;
    ...
    *Delim = c;
    ...
    return Which(PropName, sProp, 1, PROPS);
}


char* C50Classifier::RemoveQuotes(String S)
/*     ------------  */
{
    char	*p, *Start, *SS;
...
Set C50Classifier::MakeSubset(Attribute Att)
/*  ----------  */
{
    int		Bytes, b;
    char	*p;
...
Tree C50Classifier::Leaf(double *Freq, ClassNo NodeClass, CaseCount Cases)
/*   ----  */
{
    Tree	Node;
    ClassNo	c;
...
void C50Classifier::GetMCosts(FILE *Cf)
/*   ---------  */
{
...
ClassNo C50Classifier::TreeClassify(DataRec Case, Tree DecisionTree, CEnv E)
/*      ------------  */
{
    ClassNo	c, C;
    double	Prior;
...
void C50Classifier::FollowAllBranches(DataRec Case, Tree T, float Fraction, double *Prob,
                                      Boolean *AttUsed)
/*   -----------------  */
{
    DiscrValue	v;
...
void C50Classifier::FindLeaf(DataRec Case, Tree T, Tree PT, float Fraction, double *Prob,
                             Boolean *AttUsed)
/*   --------  */
{
    DiscrValue	v, Dv;
...
ClassNo C50Classifier::RuleClassify(DataRec Case, CRuleSet RS, CEnv E)
/*      ------------  */
{
    ClassNo	c, Best;
...
int C50Classifier::FindOutcome(DataRec Case, Condition OneCond)
/*  -----------  */
{
...
Boolean C50Classifier::Satisfies(DataRec Case, Condition OneCond)
/*      ---------  */
{
...
Boolean C50Classifier::Matches(CRule R, DataRec Case)
/*      -------  */
{
    int d;
...
void C50Classifier::CheckActiveSpace(int N, CEnv E)
/*   ----------------  */
{
    if ( E->ActiveSpace <= N )
    {
...
void C50Classifier::MarkActive(RuleTree RT, DataRec Case, CEnv E)
/*   ----------  */
{
    DiscrValue	v;
    int		ri;
...
ClassNo C50Classifier::BoostClassify(DataRec Case, int MaxTrial, CEnv E)
/*	-------------  */
{
    ClassNo	c, Best;
    int		t;
...
ClassNo C50Classifier::SelectClass(ClassNo Default, Boolean UseCosts, double *Prob)
/*      -----------  */
{
    ClassNo	c, BestClass;
...
double C50Classifier::MisclassCost(double *LocalFreq, ClassNo C)
/*     ------------  */
{
    double	ExpCost=0;
    ClassNo	c;
...
ClassNo C50Classifier::Classify(DataRec Case, CEnv E)
/*      --------  */
{
    E->NRulesUsed = 0;
...
float C50Classifier::Interpolate(Tree T, ContValue Val)
/*    -----------  */
{
...
FILE * C50Classifier::GetFile(String Extension, String RW)
/*    --------  */
{
    strcpy(Fn, FileStem);
    strcat(Fn, Extension);
...
void C50Classifier::CheckFile(String Extension, Boolean Write)
/*   ---------  */
{
...
char C50Classifier::ProcessOption(int Argc, char *Argv[], char *Options)
/*   -------------  */
{
    int		i;
    static int	OptNo=1;
...
void * C50Classifier::Pmalloc(size_t Bytes)
/*    -------  */
{
    void *p=Nil;
...
void * C50Classifier::Prealloc(void *Present, size_t Bytes)
/*    --------  */
{
    void *p=Nil;
...
void * C50Classifier::Pcalloc(size_t Number, unsigned int Size)
/*    -------  */
{
    void *p=Nil;
...
void C50Classifier::Error(int ErrNo, String S1, String S2)
/*   -----  */
{
...
int C50Classifier::Denominator(ContValue Val)
/*  -----------  */
{
    unsigned int	N, D=1;
    float		AltVal;
...
int C50Classifier::GetInt(String S, int N)
/*  ------  */
{
    int	Result=0;
...
int C50Classifier::DateToDay(String DS)	/*  Day 1 is 0000/03/01  */
/*  ---------  */
{
    int Year, Month, Day;
...
int C50Classifier::TimeToSecs(String TS)
/*  ----------  */
{
    int Hour, Mins, Secs;
...
void C50Classifier::SetTSBase(int y)
/*   ---------  */
{
    y -= 15;
...
int C50Classifier::TStampToMins(String TS)
/*  ------------  */
{
    int		Day, Sec, i;
    char* TSS = (char*) TS;
    ...
    for ( i = 11 ; TSS[i] && Space(TSS[i]) ; i++ )
        ;

    /*  Read time part  */

    Sec = TimeToSecs(TSS+i);
...
void C50Classifier::FreeLastCase(DataRec DVec)
/*   ------------  */
{
    free(&DVec[-1]);
    IValsOffset = 0;
...
void C50Classifier::FreeGlobals()
/*   -----------  */
{
...
void C50Classifier::FreeTree(Tree T)
/*   --------  */
{
    DiscrValue v;
...
void C50Classifier::FreeRule(CRule R)
/*   --------  */
{
    int	d;
...
void C50Classifier::FreeRuleTree(RuleTree RT)
/*   ------------  */
{
    int		b;
...
void C50Classifier::FreeRules(CRuleSet RS)
/*   ---------  */
{
    int	ri;
...
void C50Classifier::FreeVector(void **V, int First, int Last)
/*   ----------  */
{
    if ( V )
    {
...
C50Classifier::~C50Classifier() {
    FreeGlobals();
}


C50Predictor::C50Predictor(String filestem, bool rules, bool rulesused ) {
    cc.RULES = (rulesused) ? true : rules ;
    cc.RULESUSED = rulesused;
...
C50Interface::C50Interface() {
    ...
    AddParameter("filestem","model","Set the filestem for the serialized MODELFILE");
    AddParameter("database", "false", "Builds a model from the database at runtime (slow)" );
    AddParameter("trials", "-1", "Number of trials to use if building model from database" );
}
...
ErrorFlag C50Interface::BuildModelFromDatabase() {
  ...
  // the files needed to disk, then call the BuildModelFromFile function. The 
  // good thing is that this automatically results in a serialized model. 
  
  // This will create all the files in the filestem as needed. 
  std::vector< std::string > classification_labels, feature_labels;
  ...
  outfile.open( GetParameter("filestem") + ".names", std::ios::out);
  std::ostringstream oss;
  oss << *classification_labels.begin() << ".\n\n";
  ...
  bool first = true;
  for ( auto &it : solver_labels ) {
    oss << (( !first ) ? "," : "")  << it ;
    first = false;
  }
  oss << ".\n";
  oss << *classification_labels.begin() << ":\tgood,bad.\n";
  outfile << oss.str() << std::endl;
  outfile.close();

  //2. Write the datafile. 
  outfile.open( GetParameter("filestem") + ".data", std::ios::out );
  std::ostringstream oss1;
  for ( int i = 0; i < feature_data.size(); i++ ) {
     for ( auto &it : feature_data[i] ) {
        oss1 << it << "," ;
     }
     oss1 << solver_data[i] << "," << (( classification_data[i][0] ) ? "good" : "bad") << "\n"; 
  }
  outfile << oss1.str(); 
  outfile.close();

  //3. Write the solvers file. 
  outfile.open( GetParameter("filestem") + ".solvers", std::ios::out ) ;
  std::ostringstream oss2; 
  std::vector<int> solverList;
  database->GetUniqueSolverList( solverList );
  for ( auto &it : solverList ) {
      Solver s;
      database->GetUniqueSolver(it,s);
      std::string solverstring;
      s.GetSolverString(solverstring);
      oss2 << it << "," << solverstring << "\n";
  }
  outfile << oss2.str();
  outfile.close();
...
ErrorFlag C50Interface::BuildModelFromFile() {
  ...
  if ( !file.is_open() ) {
    std::cout << " <filestem>.solvers does not exist. This is needed to convert hashes to solver configs \n"; 
    std::abort();
  }
  while (!file.eof() ) {
...
ErrorFlag C50Interface::ClassifyImpl( features_map &features, Solver &solver ) {
    if ( predictor ) {
      ...
      if ( solverChoice < 0 ) 
         ...
         solver.ParseSolverString(solverMap[solverChoice]);
    }
    return 1;
}
