...
% Please, always double check the margins by measuring the margins on the screen, either using a tool that gives a pixel offset of a pointer in a PDF or a ``screen tape measure''.

\usepackage[margin=1in]{geometry} % Used to ensure that PDF latex handles the
                      % length and margin settings
                      % appropriately. http://en.wikibooks.org/wiki/LaTeX/Page_Layout
...
\setlength\voffset{-1.05in} %need a header, so need to put the header in default 1'' margins
