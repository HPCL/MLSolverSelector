...
#define EDGE 10      // These could become command line options 
#define INTERIOR 10 
#define FULL 1 
#include "FeatureExtract.h"

PetscErrorCode feature_extraction_monitor(KSP ksp, PetscInt i, PetscReal j, void* ctx) {

  // ******************************* @Kanika 
  Mat A, P;
  KSPGetOperators(ksp, &A, &P); 
  std::vector< std::pair< std::string, double >> fnames; 
  ExtractJacobianFeatures( A , EDGE, INTERIOR, fnames , true );
  printf("Printing features for solver matrix used in iteration %d\n", i );
  int rank;
...
int main(int argc,char **argv)
{
  ...
  ierr = KSPMonitorSet(ksp, &feature_extraction_monitor, NULL, NULL);
